    ...
    public sealed class OpenCL110Factory : IOpenCLFactory
    {
        ...
        public IComputeProgram BuildComputeProgram(IComputeContext context, IList<byte[]> binaries, IList<IComputeDevice> devices)
        {
            ...
            try
            {
                ...
                program.Handle = OpenCL110.CreateProgramWithBinary(
                    context.Handle,
                    count,
                    deviceHandles,
                    binariesLengths,
        ...
        {
            var kernel = new ComputeKernel110();
            kernel.Handle = OpenCL110.CreateKernel(
                program.Handle,
                functionName,
                out ComputeErrorCode error);
            ComputeException.ThrowOnError(error);
        ...
        {
            var sampler = new ComputeSampler110();
            sampler.Handle = OpenCL110.CreateSampler(context.Handle, normalizedCoords, addressing, filtering, out ComputeErrorCode error);
            ComputeException.ThrowOnError(error);
        ...
        {
            var context = new ComputeContext110();
            ...
            context.Handle = OpenCL110.CreateContext(
                propertyArray,
                handleCount,
                deviceHandles,
                notify,
            ...
            ComputeContextNotifier notify, IntPtr userDataPtr)
        {
            var context = new ComputeContext110();
            var propertyArray = context.ToIntPtrArray(properties);
            context.Handle = OpenCL110.CreateContextFromType(
                propertyArray,
                deviceType,
                notify,
                userDataPtr,
