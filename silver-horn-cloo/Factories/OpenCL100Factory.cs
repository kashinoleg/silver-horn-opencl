...
{
    public sealed class OpenCL100Factory : IOpenCLFactory
    {
        #region Services
        /// <summary>
        /// Logger
        ...
        {
            var program = new ComputeProgram100();
        ...
        {
            var sampler = new ComputeSampler100();
            sampler.Handle = OpenCL100.CreateSampler(context.Handle, normalizedCoords, addressing, filtering, out ComputeErrorCode error);
            ComputeException.ThrowOnError(error);
            ...
            ComputeContextNotifier notify, IntPtr notifyDataPtr)
        {
            ...
            context.Handle = OpenCL100.CreateContext(
                propertyArray,
                handleCount,
                deviceHandles,
                notify,
