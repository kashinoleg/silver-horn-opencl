...
namespace SilverHorn.Cloo.Context
{
    ...
    /// </example>
    public sealed class ComputeContext210 : ComputeObject, IComputeContext
    {
        #region Services
        /// <summary>
        /// Logger
        ...
        internal ReadOnlyCollection<IComputeDevice> GetDevices()
        {
            var arrayDevices = GetArrayInfo<CLContextHandle, ComputeContextInfo, CLDeviceHandle>(Handle,
                ComputeContextInfo.Devices, OpenCL210.GetContextInfoWrapper);
        ...
        public void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                if (Handle.IsValid)
                {
                    ...
                    OpenCL210.ReleaseContext(Handle);
                    Handle.Invalidate();
                }
                disposedValue = true;
            }
