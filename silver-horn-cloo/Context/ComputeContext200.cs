...
namespace SilverHorn.Cloo.Context
{
    ...
    /// </example>
    public sealed class ComputeContext200 : ComputeObject, IComputeContext
    {
        #region Services
        /// <summary>
        /// Logger
        ...
        internal ReadOnlyCollection<IComputeDevice> GetDevices()
        {
            var arrayDevices = GetArrayInfo<CLContextHandle, ComputeContextInfo, CLDeviceHandle>(Handle,
                ComputeContextInfo.Devices, OpenCL200.GetContextInfoWrapper);
        ...
        public void Dispose(bool disposing)
        {
            if (!disposedValue)
            {
                ...
                if (Handle.IsValid)
                {
                    ...
                    OpenCL200.ReleaseContext(Handle);
                    Handle.Invalidate();
                }
                disposedValue = true;
            }
